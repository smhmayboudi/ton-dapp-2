...
import (
	...
	"github.com/smhmayboudi/ton-dapp-2/crypto"
...
func (h *handler) ProofHandler(c echo.Context) error {
	...
	// check payload
	err = crypto.CheckPayload(tp.Proof.Payload, h.sharedSecret)
	if err != nil {
		return c.JSON(HttpResErrorWithLog("payload verification failed: "+err.Error(), http.StatusBadRequest, log))
	}

	parsed, err := crypto.ConvertTonProofMessage(ctx, &tp)
	if err != nil {
		return c.JSON(HttpResErrorWithLog(err.Error(), http.StatusBadRequest, log))
	}

	net := service.Networks[tp.Network]
	if net == nil {
	...
	check, err := crypto.CheckProof(ctx, addr, net, parsed)
	if err != nil {
...
func (h *handler) PayloadHandler(c echo.Context) error {
	...
	payload, err := crypto.GeneratePayload(h.sharedSecret, h.payloadTtl)
	if err != nil {
